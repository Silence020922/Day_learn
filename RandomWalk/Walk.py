...
import random
import pandas as pd
from joblib import Parallel, delayed # 简单的并行化实现库
...
class RandomWalker():
    def __init__(self, G, p=1, q=1, use_rejection_sampling=False): # 默认各可行方向概率相同
        """
        :param G: graph
        :param p: Return parameter. 回到上一位置的概率1/p
        :param q: In-out parameter,allows the search to differentiate between “inward” and “outward” nodes. outwalk的概率1/q
    ...
    def deepwalk_walk(self, walk_length, start_node): # 这是randomwoalk吧
        """
        Input:
        walk_length: short fixed length of walk process
        start_node: 起始点
        Output: walk 随机游走序列
        """
        walk = [start_node] # walk 存储随机游走序列
        while len(walk) < walk_length:
            s = walk[-1]
            s_nbs = list(self.G.neighbors(s))
            if len(s_nbs) > 0:
                walk.append(random.choice(s_nbs))
            else:
                break
        return walk
    ...
    def node2vec_walk(self, walk_length, start_node):
        ...
        while len(walk) < walk_length:
            s = walk[-1]
            s_nbs = list(G.neighbors(s))
            if len(s_nbs) > 0:
                if len(walk) == 1: # walk==1 意味着新开始
                    walk.append(
                        s_nbs[alias_sample(alias_nodes[s][0], alias_nodes[s][1])])
                else:
                    prev = walk[-2]
                    edge = (prev, s)
                    next_node = s_nbs[alias_sample(alias_edges[edge][0],
                                                      alias_edges[edge][1])]
    ...
    def node2vec_walk2(self, walk_length, start_node): # outwalk inwalk BFS-like DFS-like
        """
        Reference:
